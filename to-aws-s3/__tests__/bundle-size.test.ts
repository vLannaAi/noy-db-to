 ...
 * `dist/index.js` is ~14 KB, while `@aws-sdk/client-s3` is ~3.3 MB on disk.
 * A browser's cost is almost entirely the SDK — which is a PEER the consumer
...
if (!existsSync(distPath)) {
  throw new Error(`@noy-db/to-aws-s3: dist/index.js is missing — run \`pnpm build\` before \`pnpm test\`. `
...
/**
 * Catastrophe threshold, not a budget. The bundle is ~14 KB; inlining the
...
describe('to-aws-s3 — bundle sanity (#17)', () => {
  ...
  it('declares peers to assert against (guards the guard)', () => {
    ...
    expect(peers).toContain('@aws-sdk/client-s3')
  })
  ...
  it.each(Object.keys(pkg.peerDependencies ?? {}))('keeps %s external — imported, never inlined', (peer) => {
    ...
    // uses the former, to-aws-dynamo the latter, so both count as external.
