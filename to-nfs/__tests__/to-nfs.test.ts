...
import { toNfs, runMountDiagnostics, type MountDetector } from '../src/index.js'
...
describe('@noy-db/to-nfs — store behavior', () => {
  ...
  it('name is "nfs"', () => {
    const store = toNfs({ mountPath: dir, mountDetector: cleanDetector })
  ...
  it('nolock + onNolock: "warn" prints to console but does not throw', async () => {
    ...
    const store = toNfs({ mountPath: dir, mountDetector: nolockDetector, onNolock: 'warn' })
    await store.put('v1', 'c1', 'r1', env(1))
  ...
  it('nolock + onNolock: "error" throws', async () => {
    const store = toNfs({ mountPath: dir, mountDetector: nolockDetector, onNolock: 'error' })
    await expect(store.put('v1', 'c1', 'r1', env(1))).rejects.toThrow(/nolock/)
  })

  it('diagnostics() surfaces the raw risk list', async () => {
    const store = toNfs({ mountPath: dir, mountDetector: noacMissingDetector })
