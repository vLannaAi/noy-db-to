...
// Regression for #321 — `toAwsDynamo()` documented `casAtomic: true` (it does
...
describe('toAwsDynamo() capabilities (#321)', () => {
  it('advertises casAtomic:true (ConditionExpression CAS) with an iam auth descriptor', () => {
    const caps = toAwsDynamo({ table: 't', region: 'eu-west-1' }).capabilities
